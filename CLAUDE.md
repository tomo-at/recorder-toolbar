...
## macOS / Swift API 名の確認

enum case 名・定数名が不確かなとき（例: `CGWindowLevelKey.???`）は **推測して build してから確認する前に**、
まず `grep -r "KeywordName" /Applications/Xcode.app/...` か WebSearch で正しい名前を調べる。
推測ベースの build loop は 1 回あたり約 3 往復のロス。

---

## LGTM タスク同期

`.lgtm/tasks/` にタスクファイルがある場合、**Edit/Write/変更系 Bash を使ったターン**、
または **質問・アドバイス系タスクに実質回答したターン** の終了前に必ず:

1. `grep -rl '^status: inProgress' .lgtm/tasks/` で対象を特定
2. `summary:` が無ければ日本語1行（固有名詞入り）で書く
3. `sed -i '' '/^status: inProgress/a\summary: ...'` で書き込み

既存 summary は上書きしない。詳細は `~/.claude/skills/lgtm-task-sync.md` 参照。
